...
(
    `weather_source_id`  INT  NOT NULL AUTO_INCREMENT,
    `weather_source_url` TEXT NOT NULL,

    PRIMARY KEY (`weather_source_id`)
)
    ENGINE = InnoDB;
...
(
    `traffic_source_id`  INT  NOT NULL AUTO_INCREMENT,
...
(
    ...
    `resort_phone`          VARCHAR(45)  NOT NULL,
    ...
    CONSTRAINT `fk_Resorts_Weather_Sources1`
        FOREIGN KEY (`resort_weather_source`)
            REFERENCES `Weather_Sources` (`weather_source_id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION,

    CONSTRAINT `fk_Resorts_Traffic_Sources1`
        FOREIGN KEY (`resort_traffic_source`)
            REFERENCES `Traffic_Sources` (`traffic_source_id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION
)
    ENGINE = InnoDB;
...
(
    `slope_difficulty_id`    INT         NOT NULL AUTO_INCREMENT,
    `slope_difficulty_level` VARCHAR(45) NOT NULL,

    PRIMARY KEY (`slope_difficulty_id`)
)
    ENGINE = InnoDB;
...
(
    `slope_id`            INT          NOT NULL AUTO_INCREMENT,
    ...
    CONSTRAINT `fk_Slopes_Slope_Difficulties1`
        FOREIGN KEY (`slope_difficulty_id`)
...
(
    `chain_control_id`                INT         NOT NULL AUTO_INCREMENT,
    `chain_control_level_description` VARCHAR(45) NOT NULL,
    `chain_control_level`             VARCHAR(45) NOT NULL,

    PRIMARY KEY (`chain_control_id`)
)
    ENGINE = InnoDB;
...
(
    `road_status_category_id` INT         NOT NULL AUTO_INCREMENT,
    `road_status`             VARCHAR(45) NOT NULL,

    PRIMARY KEY (`road_status_category_id`)
)
    ENGINE = InnoDB
...
(
    `road_status_id` INT NOT NULL AUTO_INCREMENT,
    `chain_control`  INT NOT NULL,
    `road_status`    INT NOT NULL,

    PRIMARY KEY (`road_status_id`),

    CONSTRAINT `fk_Roads_Status_Categories_Chain_Control_Category1`
        FOREIGN KEY (`chain_control`)
            REFERENCES `Chain_Control_Category` (`chain_control_id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION,

    CONSTRAINT `fk_Road_Status_Road_Status_Category1`
        FOREIGN KEY (`road_status`)
            REFERENCES `Road_Status_Category` (`road_status_category_id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION
)
    ENGINE = InnoDB
...
(
    ...
    `road_name`      VARCHAR(245) NOT NULL,
    `road_direction` VARCHAR(45)  NOT NULL,
    `road_status`    INT          NOT NULL,
    ...
    CONSTRAINT `fk_Access_Roads_Roads_Statuses1`
        FOREIGN KEY (`road_status`)
            REFERENCES `Road_Status` (`road_status_id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION
)
    ENGINE = InnoDB;
...
(
    `resort_id`      INT     NOT NULL,
    `access_road_id` INT     NOT NULL,
    `is_to_resort`   TINYINT NOT NULL,
    `is_from_resort` TINYINT NOT NULL,
    ...
    CONSTRAINT `fk_Resorts_has_Access_Roads_Access_Roads1`
        FOREIGN KEY (`access_road_id`)
            REFERENCES `Access_Roads` (`road_id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION
)
    ENGINE = InnoDB;
